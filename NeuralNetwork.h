...
public:
	...
	{
		float trainSize = (float)fmaxf(testSize / (1 + (dataLength) * testSize > 1), 0);
		trainSize = 1 - testSize;

		DataManipulation::ShuffleData(X, dataLength);
		DataManipulation::ShuffleData(Y, dataLength);

		size_t slicePoint = DataManipulation::GetSlicePoint(dataLength, trainSize);

		float testCost = 50E30f;
		size_t batchCount = slicePoint / batchSize;
		size_t lastBatchSize = slicePoint % batchSize;
		while (testCost > trainUntilTestCostIsBelow)
		{
			for (size_t i = 0; i < batchCount; i++)
			{
				SupervisedLearningBatch(X, Y, batchSize * i, batchSize, costFunction, learningRate);
			}
			SupervisedLearningBatch(X, Y, batchCount * batchSize, lastBatchSize, costFunction, learningRate);

			testCost = 0;
			for (size_t i = slicePoint; i < dataLength; i++)
			{
				testCost += Cost::GetCostOf(OutputLength, Execute(X[i]), Y[i], costFunction);
			}
			testCost /= dataLength - slicePoint;
		}
	}
...
private:
	...
	public:
		...
		{
			tuple<float, float*> gradients = (*neuronIterator)->GetGradients(networkActivations, linearFunction, neuronCost, networkCosts, activationFunction);
